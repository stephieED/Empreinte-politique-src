...
Matrix strategy depuis #344 (voir `docs/decisions/matrix-extract-an-par-candidat.md`) :
...
(angle mort documenté dans `docs/decisions/resilience-generate-data-shutdown-signal.md`
...
5. Les amendements AN sont indexés par `acteurRef` (`PAxxxx`), avec mapping des états procéduraux — lus exclusivement depuis `.cache/amendements_an/` (jamais téléchargés par ce job, voir `docs/decisions/amendements-index-cache-only-consumers.md`).
