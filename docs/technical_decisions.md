...
- **l'intégrité référentielle** entre un `votes[].scrutin_id` d'un profil et
  ...
  index. Un mapping peut pointer dans le vide sans qu'aucun compteur ne bouge.
  C'est le trou le plus sérieux qui reste, et il est hors de portée d'un
  contrôle à mémoire bornée : il faudrait tenir les deux ensembles de clés en
