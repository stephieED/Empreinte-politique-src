...
`tests/test_syceron_acteur_ref.py` (20 tests) : la forme réelle de l'archive, la
normalisation cas par cas, l'inactivité par défaut, la séparation des deux
fichiers d'index, la garde §2.5 sur index vide, et deux tests qui **figent les
deux défauts de parseur** ci-dessus — le jour où le parcours devient récursif ou
où `sujet` se remplit, ils échouent et rappellent que la volumétrie de la
décision d'activation doit être refaite.
