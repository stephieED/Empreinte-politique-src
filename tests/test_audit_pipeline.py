...
def test_compute_vue_ensemble_agrege_les_totaux():
    ...
    vue = compute_vue_ensemble(rapport_profils, rapport_groupes)

    assert vue["total_profils_audites"] == 3
    assert vue["total_groupes_audites"] == 2
    assert vue["total_erreurs_lecture"] == 2
...
def test_compute_vue_ensemble_fusionne_les_warnings_communs_aux_deux_audits():
    rapport_profils = _rapport_profils(
        warnings_par_type={
    ...
    assert vue["warnings"]["total_warnings"] == 3
...
def test_compute_vue_ensemble_type_de_warning_present_dans_un_seul_audit():
    ...
    assert vue["warnings"]["par_type"]["seulement_profils"] == {
        "frequence": 1, "profils_ids": ["nosdeputes:a"], "groupe_ids": [],
    }
    assert vue["warnings"]["par_type"]["seulement_groupes"] == {
        "frequence": 1, "profils_ids": [], "groupe_ids": ["AN:LFI"],
    }


def test_compute_vue_ensemble_sans_warning_ni_erreur():
    vue = compute_vue_ensemble(_rapport_profils(), _rapport_groupes())

    assert vue["warnings"] == {"total_warnings": 0, "par_type": {}}
    assert vue["erreurs_lecture"] == {"profils": [], "groupes": []}
...
def test_build_report_sur_fixtures_reelles():
    ...
    assert rapport["vue_ensemble"]["total_erreurs_lecture"] == 2
    assert rapport["vue_ensemble"]["warnings"]["total_warnings"] == 1
...
def test_generate_markdown_report_contient_toutes_les_sections():
    rapport = build_report(_rapport_profils(), _rapport_groupes())
    ...
    assert "# Rapport d'audit pipeline (profils + groupes)" in markdown
