...
| **Frise de couverture (`/couverture`)** | **Un rail, N couches** — jamais une piste par origine (`.fc-rail` / `.fc-couche`, #328). Trois fiches peuvent porter le même fait sur la même période : elles se superposent en `mix-blend-mode: multiply`, une origine seule garde sa teinte, deux qui se recouvrent en produisent une troisième plus sombre. Les trois teintes disent **de quelle fiche vient le trait** : `--pop-cand` `#3F5166` (candidats), `--pop-gouv` `#8A6B4C` (gouvernements), `--pop-grp` `#6F5B7A` (groupes) — les deux premières sont celles de la fiche candidat, la troisième les complète. **L'institution est la hiérarchie, l'origine est la teinte** : en faire un niveau mettrait trois lignes là où il y a un fait. Deux absences, deux motifs, jamais confondus (§6 bis règle 7) — la **hachure pâle** dit « la source ne publie pas » (un fait sur l'institution, immuable), le **jaune hachuré à filet d'encre** dit « nous ne l'avons pas collecté » (un fait sur NOUS, et l'accent marque l'action dans ce système). |
