...
| **Borne par institution (accueil)** | En tête de « Sources & fraîcheur des données » (`CouvertureAccueil.jsx`, `.ca`, #328) : une ligne par institution dans sa teinte, « depuis » et l'année ; Sénat en jaune « non collecté », mandats locaux en hachure « aucune source ». La hachure d'une institution n'est dessinée que si toutes ses listes déclarent une borne. En dessous, « Les mandats hors couverture » nomme les fiches concernées, chacune liée à sa page — tout vient de `couverture.json` (`accueil`), rien n'est écrit dans le JSX. Sous 560 px, une graduation sur deux. |
