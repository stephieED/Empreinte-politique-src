...
**Gouvernement** (real governments) — no Partis tab. Three tabs, but **five published
pages**: `/methodologie`, `/mentions-legales` and `/couverture` live outside the tabs.
`/couverture` says once, for the whole corpus, what every fiche used to repeat — what the
...
| Répertoire national des élus (data.gouv.fr, `tabular-api`) | **Not collected yet — cited** (#922): the reviewed table `raw_data/correspondance_elus_rne.json` carries the matching key (name, first name, date of birth) and the commune as the RNE writes them, for declared candidates the corpus has no date of birth for. Collecting the local mandates themselves is a later lot | Licence Ouverte 2.0 (Etalab) | Attribution only |
