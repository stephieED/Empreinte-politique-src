...
| EuroVoc (publications.europa.eu, point SPARQL) | **Yes since #901 — one property, `skos:prefLabel` in French**, to name a concept whose identifier the European Parliament already gave. Nothing is discovered here and nothing is classified here: the subject of a document is the Parliament's fact, never our reading of its title | CC BY 4.0 | Attribution, and changes must be stated. A code the thesaurus does not return is **never invented**: the document keeps the labels found and declares the others |
